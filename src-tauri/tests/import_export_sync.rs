...
fn sync_codex_provider_writes_config_without_touching_auth() {
    ...
    let auth_path = hyper_mitm_lib::get_codex_auth_path();
    let config_path = hyper_mitm_lib::get_codex_config_path();
...
requires_openai_auth = true
"#;
    hyper_mitm_lib::write_codex_live_atomic(&legacy_auth, Some(legacy_config))
...
"#
    ...
    let toml_text =
        fs::read_to_string(hyper_mitm_lib::get_codex_config_path()).expect("read config.toml");
...
fn sync_enabled_to_codex_writes_enabled_servers() {
    ...
    let path = hyper_mitm_lib::get_codex_config_path();
    ...
    hyper_mitm_lib::sync_enabled_to_codex(&config).expect("sync codex");
...
fn sync_enabled_to_codex_returns_error_on_invalid_toml() {
    ...
    let err = hyper_mitm_lib::sync_enabled_to_codex(&config).expect_err("sync should fail");
    match err {
        hyper_mitm_lib::AppError::Toml { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "path should reference config.toml"
            );
        }
        hyper_mitm_lib::AppError::McpValidation(msg) => {
            assert!(
...
fn sync_codex_provider_missing_auth_returns_error() {
    ...
    match err {
        hyper_mitm_lib::AppError::Config(msg) => {
    ...
    // 确认未产生任何 live 配置文件
    assert!(
        !hyper_mitm_lib::get_codex_auth_path().exists(),
        "auth.json should not be created on failure"
    );
    assert!(
        !hyper_mitm_lib::get_codex_config_path().exists(),
...
"#;
    ...
    hyper_mitm_lib::write_codex_live_atomic(&auth, Some(config_text))
    ...
    let stored_auth: serde_json::Value =
        hyper_mitm_lib::read_json_file(&auth_path).expect("read auth");
...
"#;
    ...
    let err = hyper_mitm_lib::write_codex_live_atomic(&auth, Some(config_text))
        .expect_err("config write should fail when target is directory");
    match err {
        hyper_mitm_lib::AppError::Io { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "io error path should point to config.toml"
            );
        }
        hyper_mitm_lib::AppError::IoContext { context, .. } => {
            assert!(
...
"#,
    ...
    let mut config = MultiAppConfig::default();
    let changed = hyper_mitm_lib::import_from_codex(&mut config).expect("import codex");
...
fn sync_claude_enabled_mcp_projects_to_user_config() {
    ...
    hyper_mitm_lib::sync_enabled_to_claude(&config).expect("sync Claude MCP");

    let claude_path = hyper_mitm_lib::get_claude_mcp_path();
...
fn import_from_claude_merges_into_config() {
    ...
    let changed = hyper_mitm_lib::import_from_claude(&mut config).expect("import from claude");
