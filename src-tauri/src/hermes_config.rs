...
///
/// Input (from Hyper MITM UI / database):
/// ```json
...
/// `providers:` dict. Hyper MITM treats those as read-only — writes have to
...
/// - `custom_providers:` list entries (writable by Hyper MITM)
...
/// keeping Hyper MITM free to edit it. Models are denormalized from the YAML
...
/// list — i.e. it is a read-only overlay Hyper MITM must not touch.
...
pub fn set_provider(
        ...
        .find(|p| p.get("name").and_then(|n| n.as_str()) == Some(name))
    {
        ...
        // this merge, a Hyper MITM edit to an unrelated field would silently
...
// Hyper MITM fills that gap by reading/writing the whole file as a markdown
...
";
    ...
    fn set_provider_preserves_unknown_fields_on_update() {
        ...
        // Hermes Web UI; a later Hyper MITM edit must not strip them — set_provider
...
";
    ...
    fn set_memory_enabled_preserves_other_fields() {
        ...
        // a Hyper MITM toggle would silently wipe those fields.
        with_test_home(|| {
            let yaml = "\
memory:
