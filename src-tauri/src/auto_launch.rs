...
/// 将 `/path/to/Hyper MITM.app/Contents/MacOS/Hyper MITM` 转换为 `/path/to/Hyper MITM.app`
#[cfg(target_os = "macos")]
...
mod tests {
    ...
    fn test_get_macos_app_bundle_path_valid() {
        let exe_path = std::path::Path::new("/Applications/Hyper MITM.app/Contents/MacOS/Hyper MITM");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from("/Applications/Hyper MITM.app"))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_with_spaces() {
        let exe_path =
            std::path::Path::new("/Users/test/My Apps/Hyper MITM.app/Contents/MacOS/Hyper MITM");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from(
                "/Users/test/My Apps/Hyper MITM.app"
            ))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_not_in_bundle() {
        let exe_path = std::path::Path::new("/usr/local/bin/hyper-mitm");
        let result = get_macos_app_bundle_path(exe_path);
    ...
    fn test_get_macos_app_bundle_path_dev_build() {
        ...
        let exe_path = std::path::Path::new("/Users/dev/project/target/debug/hyper-mitm");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(result, None);
    }
}
