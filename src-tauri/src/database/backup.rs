...
const CC_SWITCH_SQL_EXPORT_HEADER: &str = "-- Hyper MITM SQLite 导出";
...
impl Database {
    ...
    fn validate_cc_switch_sql_export(sql: &str) -> Result<(), AppError> {
        ...
        Err(AppError::localized(
            ...
            "仅支持导入由 Hyper MITM 导出的 SQL 备份文件。",
            "Only SQL backups exported by Hyper MITM are supported.",
        ))
    }
